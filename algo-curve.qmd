...
To compare the computation time, we use the `R` package `microbenchmark` version 1.5.0 [@Mersmann:2024aa] with `R` version 4.4.0 (2024-04-24) and `sanssouci` version 0.13.0, on a MacBook Air M1 (2020) running macOS 15.1.1. The package `microbenchmark` allows to run code snippets a given number `n_repl` of times, and to compute summary statistics on the computation time.

Four scenarios are studied, all based on a common setting which we first describe.
