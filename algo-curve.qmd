...
Now assume that we have the following values for the $\zeta_k$'s: $\zeta_{(1,5)}=6$, $\zeta_{(1, 1)}=2$, $\zeta_{(2, 3)}=1$, $\zeta_{(3, 3)}=4$, $\zeta_{(4, 5)}=4$, $\zeta_{(4, 4)}=2$, $\zeta_{(5, 5)}=3$, $\zeta_{(6, 7)}=2$, $\zeta_{(7, 7)}=0$. Because $P_2$, $P_6$ and $P_8$ come from the completion operation, we also have $\zeta_{(2, 2)}=|P_2|=2$, $\zeta_{(6, 6)}=|P_6|=1$ and $\zeta_{(8, 8)}=|P_8|=3$. Theses values are summarized in @fig-zetas.
