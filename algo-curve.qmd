...
$$ {#eq-vstar-Qpartition}
provided that the family is complete. Here, $\mathfrak P \subseteq \mathcal P(\cK)$ is the set of subsets of $\cK$ that realize a partition, that is, the set of $Q\subseteq\cK$ such that the $R_k$, $k\in Q$, form a partition of $\Nm^*$. So the minimum in @eq-vstar-Qpartition is over way less elements than in @eq-vstar-Q.
...
In the practical implementation of this algorithm (and of the following @alg-pruning), $Vec$ and $newVec$ are always of size $N$ (the number of leaves) instead of the cardinality of $\cK^h$. And the sum $\sum_{k'\in Succ_k} Vec_{k'}$ is really easy to compute: if $R_k= R_{(i_0,i_{p+1}-1)}= \bigcup_{ j=0}^p R_{(i_{ j}, i_{ j+1}-1)}=\bigcup_{i_0\leq n\leq i_{p+1}-1}P_n\in\cK^h$ for some $p\geq1$, a strictly increasing sequence $(i_0,\dotsc,i_{p+1})$ and $R_{(i_{ j}, i_{ j+1}-1)}\in\cK^{h+1}$ for all $0\leq j\leq p$, then we simply sum $Vec$ over the indices from $i_{0}$ to $i_{p+1}-1$. After that, the computed quantity is set in $newVec$ at index $i_0$. So actually computing $Succ_k$ is not needed and not done.
:::

::: {#rem-zeta}

The specific computation of the $R_k$'s and the $\zeta_k$'s such that @eq-jer-control holds is outside the scope of the present paper, but different computations can be found in @MR4124323, @MR4178188 or <!--notip--> for example.
