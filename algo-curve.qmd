...
\begin{algorithmic}
\Procedure{Vstar}{$\mathfrak{R}=(R_{k},\zeta_{k})_{k\in\mathcal{K}}$  with $\mathfrak{R}$ complete}
...
In the practical implementation of this algorithm (and of the following @alg-pruning), $Vec$ and $newVec$ are always of size $N$ (the number of leaves) instead of the cardinality of $\cK^h$. And the sum $\sum_{k'\in Succ_k} Vec_{k'}$ is really easy to compute: if $R_k= R_{(i_0,i_{p+1}-1)}= \bigcup_{ j=0}^p R_{(i_{ j}, i_{ j+1}-1)}=\bigcup_{i_0\leq n\leq i_{p+1}-1}P_n\in\cK^h$ for some $p\geq1$, a strictly increasing sequence $(i_0,\dotsc,i_{p+1})$ and $R_{(i_{ j}, i_{ j+1}-1)}\in\cK^{h+1}$ for all $0\leq j\leq p$, then we simply sum $Vec$ over the indices from $i_{0}$ to $i_{p+1}-1$. So actually computing $Succ_k$ is not needed and not done.
:::
